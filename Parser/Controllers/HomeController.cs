...
public class HomeController : Controller
{
    ...
    public async Task<IActionResult> Appender(HomeAppenderModel model)
    {
        ...
        {
            return View(model);  // Возвращаем пользователя обратно, если модель невалидна
        }

        // Пытаемся найти существующее название товара
        ...
        // После добавления и парсинга данных перенаправляем пользователя на DataViewer с результатами
    ...
    public async Task<IActionResult> Delete(int id)
    {
        try
        {
            ...
            else
            {
                ...
                TempData["ErrorMessage"] = $"Запись с ID {id} не найдена.";
            }
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Ошибка при удалении записи с Id {Id}.", id);
            TempData["ErrorMessage"] = $"Произошла ошибка при удалении записи с ID {id}.";
        }
    ...
    public async Task<IActionResult> Delete2(int id)
    {
        ...
        if (variant != null)
        {
            var productId = variant.ProductId; // Сохраняем ProductId для редиректа
            _context.priceLogs.Remove(variant);
            await _context.SaveChangesAsync();
            _logger.LogInformation("Успешно удалено: {Id}", id);
            return RedirectToAction("Results", new { id = productId });
        }
        else
        {
            _logger.LogWarning("Попытка удаления: запись с Id {Id} не найдена.", id);
